...
function monthLabel(){
 ...
 return new Date(`${m}-01T12:00:00`).toLocaleDateString('pt-BR',{month:'long',year:'numeric'});
}
function mount(){
 if(document.getElementById('p360DashboardAI'))return;
 const root=document.createElement('div');root.id='p360DashboardAI';root.className='p360-dashboard-ai';
 root.innerHTML=`<button class="ai-dash-launcher" title="Abrir IA Financeira" aria-label="Abrir IA Financeira">✦</button><div class="ai-dash-panel"><div class="ai-dash-head"><div class="ai-dash-orb">✦</div><div class="ai-dash-title"><strong>Assistente Patrimônio 360</strong><span>IA financeira · ${esc(monthLabel())}</span></div><span class="ai-dash-online"><i></i>P360 Intelligence online</span><button class="ai-dash-icon ai-dash-min" aria-label="Minimizar">−</button></div><div class="ai-dash-context"><span>Contexto do Dashboard</span><small>Usando receitas, despesas, orçamento, contas, cartões, investimentos, patrimônio e metas oficiais.</small></div><div class="ai-dash-messages"><div class="ai-dash-msg assistant"><div class="ai-dash-bubble">Olá! Estou dentro do seu Dashboard. Posso analisar sua situação financeira, metas, gastos, compras e compromissos usando os dados oficiais da sua conta.</div></div></div><div class="ai-dash-suggestions"><button data-q="Como estão minhas finanças?">Como estão minhas finanças?</button><button data-q="Estou no caminho da minha meta?">Estou no caminho da minha meta?</button><button data-q="Analise meus gastos.">Analise meus gastos</button><button data-q="Posso fazer uma compra?">Posso fazer uma compra?</button><button data-q="Quanto posso gastar?">Quanto posso gastar?</button><button data-q="Simule uma compra de R$ 1.200 em 5x">Simule uma compra</button></div><form class="ai-dash-form"><textarea rows="1" placeholder="Pergunte à IA..." aria-label="Mensagem para a IA"></textarea><button class="ai-dash-send" type="submit">Enviar</button></form></div>`;
 ...
 function syncVisibility(){
   ...
   root.style.display=dashboardVisible?'':'none';
   if(!dashboardVisible)close();
   else if(root.classList.contains('is-minimized')===false)open();
 }
 syncVisibility();
