...
export async function respondV2({question,state,currentMonth,memory,analysis,risk,profile,insights}){
 ...
 else if(intent==='goal'){const parsed=parseGoal(question,new Date());const existing=parsed?.target?(state.goals||[]).find(g=>Math.abs(Number(g.target||0)-parsed.target)<0.01&&(!parsed.deadline||String(g.date||g.deadline||'').startsWith(parsed.deadline))):null;if(parsed?.target){if(existing)answer=`Já existe uma meta oficial de ${money(parsed.target)} cadastrada. Vou usá-la como referência nas próximas análises.`;else{const createdAt=new Date().toISOString();mutation={type:'createGoal',goal:{id:Date.now(),name:`Meta de ${money(parsed.target)}`,current:0,target:parsed.target,date:parsed.deadline||'',priority:'Alta',createdAt,updatedAt:createdAt}};answer=`Registrei a meta de ${money(parsed.target)}${parsed.deadline?` até ${parsed.deadline}`:''}. Ela passa a ser considerada nas análises de compras, aportes e fluxo.`;}}else answer='Posso acompanhar a meta, mas preciso do valor-alvo. Exemplo: “Quero chegar a R$ 5.000 até dezembro”.';}
