...
export function detectIntent(question, memory = {}) {
  ...
  const naturalPurchase=/(?:quero|queria|gostaria|t[ôo]|to|tô|estou)\s+(?:comprar|pegar|trocar|adquirir)|(?:t[ôo]|to|tô|estou)\s+(?:pensando|de olho|querendo)\s+(?:em\s+)?(?:comprar|pegar|trocar)|(?:ser[aá]|d[aá]|da pra|d[aá] para)\s+(?:eu\s+)?(?:comprar|pegar)|(?:vale a pena|ser[aá] que d[aá])\s+(?:comprar|pegar)|(?:achei|encontrei|vi)\s+(?:um|uma)\s+(?:celular|notebook|tv|televis[aã]o|produto)|(?:vou|quero)\s+(?:pegar|ficar)\s+(?:um|uma)\s+(?:celular|notebook|tv|produto)/;
