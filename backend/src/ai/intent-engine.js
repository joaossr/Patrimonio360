...
function classifyFallback(q){
  ...
  const budget=/orcamento|margem|folga|quanto posso gastar|quanto ainda posso|quanto sobra|organizar minha vida financeira|organizar.*financas|organizar.*orcamento|por onde comeco/;
...
export function detectIntent(question, memory = {}) {
  ...
  const naturalPurchase=/(?:quero|queria|gostaria|t[ôo]|to|tô|estou)\s+(?:comprar|pegar|trocar|adquirir)|(?:t[ôo]|to|tô|estou)\s+(?:pensando|querendo)\s+(?:em\s+)?(?:comprar|pegar|trocar)|(?:t[ôo]|to|tô|estou)\s+de\s+olho\s+(?:em|num|numa)\s+(?:um|uma)?\s*(?:celular|notebook|tv|produto)|(?:ser[aá]|d[aá]|da pra|d[aá] para)\s+(?:eu\s+)?(?:comprar|pegar)|(?:vale a pena|ser[aá] que d[aá])\s+(?:comprar|pegar)|(?:achei|encontrei|vi)\s+(?:um|uma)\s+(?:celular|notebook|tv|televis[aã]o|produto)|(?:vou|quero)\s+(?:pegar|ficar)\s+(?:um|uma)\s+(?:celular|notebook|tv|produto)|(?:vou|quero)\s+(?:fazer|pegar)\s+em\s+\d+\s+(?:vezes|parcelas?)/;
  if (naturalPurchase.test(q)) return 'purchase';
  ...
  const goalProjection=/(quanto|qual|como|quando|prazo|tempo).*(guardar|poupar|economizar|aportar|aporte|meta|objetivo)|quanto falta.*(meta|objetivo)|projec[aã]o|projetar.*meta|em quanto tempo.*(chego|atingir|alcan[cç]ar)|ate dezembro|até dezembro/;
  ...
  if (/orcamento|orçamento|quanto posso gastar|margem|folga|quanto ainda posso|quanto sobra|organizar minha vida financeira|organizar.*financas|organizar.*orcamento|por onde comeco/.test(q)) return 'budget';
