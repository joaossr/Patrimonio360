...
const cases = [
  ['1200', 1200, 1, 1200], ['1.200', 1200, 1, 1200], ['1200,50', 1200.5, 1, 1200.5], ['1,2 mil', 1200, 1, 1200], ['R$ 1.200,50', 1200.5, 1, 1200.5],
  ['1200 em 5x', 1200, 5, 240], ['1200 em 5 parcelas', 1200, 5, 240], ['1200 em cinco vezes', 1200, 5, 240], ['5x de 240', 1200, 5, 240], ['5 parcelas de 240', 1200, 5, 240]
];
for (const [text,total,installments,monthly] of cases) {
  ...
  assert.equal(parsed.installments,installments,`parcelas: ${text}`);
  assert.equal(parsed.installmentValue,monthly,`parcela: ${text}`);
}
...
const intentCases = [
  ['oi','greeting'], ['Olá, tudo bem?','greeting'], ['bom dia','greeting'], ['boa noite!','greeting'], ['e aí, tudo certo?','greeting'],
  ['o que você consegue fazer?','capabilities'], ['me explica rapidamente suas principais funções','capabilities'],
...
assert.equal(detectIntent('Ele custa R$ 1.200.',conversationMemory),'purchase');

const contextualExpenseMemory={recent:[{role:'user',content:'gastei 100 hoje'}]};
