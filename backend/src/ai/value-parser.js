...
function candidates(text) {
  ...
  const re = /(?:r\$\s*)?(\d{1,3}(?:\.\d{3})+(?:,\d{1,2})?|\d+(?:,\d{1,2})?|\d+(?:\.\d{1,2})?)/gi;
...
export function parseInstallments(text) {
  ...
  for (const pattern of [
    /(?:em|de|por)\s*(\d{1,3})\s*(?:x|vezes|parcelas?)/,
    /\b(\d{1,3})\s*x\b/,
...
export function parseMoney(text) {
  ...
  if (thousand) {
    const n = normalizeMoney(thousand[1]);
    if (n) return n * 1000;
  }
...
export function parseGoal(text, currentDate = new Date()) {
  const raw = String(text || ''), normalized = raw.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '');
  ...
  let month = null;
  for (let i = 0; i < MONTHS.length; i++) if (normalized.includes(MONTHS[i])) month = i + 1;
  if (!month) return { target, month: null, year: null, deadline: null };
  ...
  const year = explicitYear ? Number(explicitYear[1]) : month < currentMonth ? currentYear + 1 : currentYear;
  return { target, month, year, deadline: `${year}-${String(month).padStart(2, '0')}` };
}
