...
function backendStatus(neural){const telegramConfigured=Boolean(process.env.TELEGRAM_BOT_TOKEN);return{ok:firebaseReady&&neural.available,service:'P360 Intelligence',backend:{port:Number(process.env.PORT||8787)},firebase:{ready:firebaseReady,projectId:process.env.FIREBASE_PROJECT_ID||null,error:firebaseInitError?.message||null},telegram:{configured:telegramConfigured,username:process.env.TELEGRAM_BOT_USERNAME||'Patrimonio360Bot'},provider:'p360-neural-engine',financialEngine:'v2',memoryEngine:'v2',languageEngine:'p360-language-v2',model:neural.available?{available:true,version:neural.modelVersion}:{available:false,reason:neural.reason}};}
