...
try {
    if (!getApps().length) {
        ...
        if (serviceAccountJson) credential = cert(JSON.parse(serviceAccountJson));
            ...
            if (!fs.existsSync(serviceAccountPath)) throw new Error(`Credencial Firebase nao encontrada em: ${serviceAccountPath}`);
    ...
    console.error('P360 Firebase nao inicializou:', error.message);
}
...
async function authUser(req, res, next) {
    if (!firebaseReady) return res.status(503).json({ error: 'Firebase Admin nao configurado.', details: firebaseInitError?.message || 'Credencial ausente.' });
    try {
...
async function loadState(uid) {
    if (!firebaseReady || !db) throw new Error('Firebase Admin nao configurado.');
...
async function saveState(uid, state) {
    if (!firebaseReady || !db) throw new Error('Firebase Admin nao configurado.');
    await db.doc(`users/${uid}/app/state`).set({ state, updatedAt: new Date().toISOString() }, { merge: true });
}
...
function backendStatus(neural) {
    ...
    return {
        ...
        telegram: { configured: telegramConfigured, username: process.env.TELEGRAM_BOT_USERNAME || 'Patrimonio360Bot' },
        provider: 'p360-neural-engine', financialEngine: 'v2', memoryEngine: 'v2', languageEngine: 'p360-language-v2',
...
app.get('/api/status', async (req, res) => {
    try { const neural = await neuralForecast({}, new Date().toISOString().slice(0, 7)); const status = backendStatus(neural); res.status(status.ok ? 200 : 503).json(status); }
    catch (error) { console.error('P360 status error:', error); res.status(503).json({ ok:false, service:'P360 Intelligence', error:'Falha ao verificar o backend.' }); }
});
app.get('/health', async (req, res) => {
    try { const neural = await neuralForecast({}, new Date().toISOString().slice(0, 7)); const status = backendStatus(neural); res.status(status.ok ? 200 : 503).json(status); }
    catch (error) { console.error('P360 health error:', error); res.status(503).json({ ok:false, service:'P360 Intelligence', error:'Falha ao verificar o backend.' }); }
});

app.post('/api/telegram/link-code', authUser, async (req, res) => {
    try { res.json(await createTelegramLinkCode(db, req.user.uid)); }
    catch (error) { console.error('Telegram link-code error:', error); res.status(500).json({ error:'Não foi possível gerar o código do Telegram.' }); }
});
...
app.post('/api/ai/chat', authUser, async (req, res) => {
    try {
        ...
        const currentMonth = req.body.selectedMonth || state.selectedMonth || new Date().toISOString().slice(0, 7);
        const analysis = analyzeFinancialState(state, currentMonth);
        const risk = calculateFinancialRisk(analysis);
        const neural = await neuralForecast(state, currentMonth);
        const profile = buildFinancialProfile(state);
        const storedMemory = await loadAIMemory(db, req.user.uid);
        const clientRecent = Array.isArray(req.body.recent) ? req.body.recent.slice(-20) : [];
        const memory = mergeMemory(storedMemory, { recent: clientRecent.length ? clientRecent : (storedMemory.recent || []) });
        const insights = generateInsights(analysis, risk, neural, profile);

        const v2 = respondV2({ question: message, state, currentMonth, memory, analysis, risk, profile, insights });
        let answer = v2.answer;
        let nextState = state;
        if (v2.mutation?.type === 'createGoal') {
            nextState = { ...state, goals: [v2.mutation.goal, ...(state.goals || [])] };
            await saveState(req.user.uid, nextState);
        }
        ...
        const enriched = mergeMemory(memory, {
            recent,
            ...v2.memoryPatch,
            context: { ...(memory.context || {}), lastQuestion:message, previousQuestion:memory.recent?.filter(x=>x.role==='user').slice(-1)[0]?.content || '', lastIntent:v2.intent, lastMonth:v2.month, updatedAt:new Date().toISOString() },
            goals: nextState.goals?.filter(g=>g.id===v2.mutation?.goal?.id).map(g=>({id:g.id,name:g.name,target:g.target,date:g.date,source:'official-state'})) || []
        });
        await Promise.all([
            saveAIMemory(db, req.user.uid, enriched),
            saveFinancialProfile(db, req.user.uid, profile),
            saveInsights(db, req.user.uid, insights)
        ]);
        res.json({ answer, intent:v2.intent, month:v2.month, engine:{ version:'v2', analysis, risk, neural, profile, insights, memory:{ recentCount:recent.length, facts:enriched.facts?.length||0, goals:enriched.goals?.length||0, corrections:enriched.corrections?.length||0 }, context:v2.context } });
    } catch (error) {
...
app.listen(Number(process.env.PORT || 8787), () => {
    ...
    if (telegramPollingEnabled) startTelegramBot({ db, loadState });
    else console.log('P360 Telegram polling desativado nesta instância.');
});
