...
import { applyConversationPolicy } from './ai/conversation-policy.js';
import { validateResponse, repairResponse } from './ai/response-validator.js';
...
app.post('/api/ai/chat',authUser,async(req,res)=>{try{const message=String(req.body.message||'').trim().slice(0,4000);if(!message)return res.status(400).json({error:'Mensagem vazia.'});const state=await loadState(req.user.uid),currentMonth=req.body.selectedMonth||state.selectedMonth||new Date().toISOString().slice(0,7),analysis=analyzeFinancialState(state,currentMonth),risk=calculateFinancialRisk(analysis),neural=await neuralForecast(state,currentMonth),profile=buildFinancialProfile(state),storedMemory=await loadAIMemory(db,req.user.uid),clientRecent=Array.isArray(req.body.recent)?req.body.recent.slice(-20):[],memory=mergeMemory(storedMemory,{recent:clientRecent.length?clientRecent:(storedMemory.recent||[])}),insights=generateInsights(analysis,risk,neural,profile);
const policy=applyConversationPolicy({question:message,state,month:currentMonth,memory,analysis});
let v2;
if(policy){v2={answer:policy.answer,intent:policy.intent||'general',month:currentMonth,context:{conversationPolicy:true},memoryPatch:{}};}
else{v2=await respondV2({question:message,state,currentMonth,memory,analysis,risk,profile,insights});}
let answer=v2.answer,nextState=state;
...
const validation=validateResponse({answer,context:v2.context});if(!validation.valid)answer=repairResponse(answer,v2.context);
const recent=[...(memory.recent||[]).slice(-18),{role:'user',content:message},{role:'assistant',content:answer}].slice(-20);
const enriched=mergeMemory(memory,{recent,...v2.memoryPatch,context:{...(memory.context||{}),lastQuestion:message,previousQuestion:memory.recent?.filter(x=>x.role==='user').slice(-1)[0]?.content||'',lastIntent:v2.intent,lastMonth:v2.month,updatedAt:new Date().toISOString()},goals:(nextState.goals||[]).map(g=>({id:g.id,name:g.name,target:g.target,date:g.date,source:'official-state'}))});
...
res.json({answer,intent:v2.intent,month:v2.month,engine:{version:'v2',analysis,risk,neural,profile,insights,memory:{recentCount:recent.length,facts:enriched.facts?.length||0,goals:enriched.goals?.length||0,corrections:enriched.corrections?.length||0},context:v2.context,validation:{valid:validation.valid,repaired:!validation.valid,errors:validation.errors}}});}catch(error){console.error('P360 AI chat error:',error);res.status(500).json({error:'Não foi possível gerar a análise agora.'});}});
