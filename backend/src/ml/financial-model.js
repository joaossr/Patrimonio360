...
export function financialFeatures(context = {}, benchmark = {}) {
  ...
  return [income/15000, expense/15000, Math.max(-0.5,Math.min(1,1-expense/safeIncome)), Math.min(reserve/safeIncome/6,2)/2, Math.min(debt/safeIncome,1), Math.min(installments/safeIncome,1), target>0?Math.max(0,Math.min(1,current/target)):1, Math.max(0,Math.min(1,1-deadlineMonths/24)), Math.min(discretionary/safeIncome,1), Math.max(0,Math.min(1,stability)), Math.max(-1,Math.min(1,benchmarkDeviation))];
}

export function loadFinancialModel() {
  try { model = JSON.parse(fs.readFileSync(MODEL_PATH,'utf8')); return true; }
  catch { model = null; return false; }
}

export function predictFinancialModel(features, loadedModel=model) {
  if(!loadedModel)return null;
  const mean=loadedModel.normalization?.mean||[], std=loadedModel.normalization?.std||[];
...
export function scoreFinancialContext(context={},benchmark={}) {
  if(!model)loadFinancialModel();
  if(!model)return {available:false,reason:'Modelo financeiro próprio ainda não foi treinado. Execute npm run train:financial.'};
